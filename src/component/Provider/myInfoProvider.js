...
function MyInfoProvider(props) {
  ...
  const checkedWallet = async () => {
    ...
    if (!currentWeb)
      return -1;
    console.log("compare wallet", window.tronWeb.defaultAddress.hex, userInfo.wallet)
    if (walletCompare(window.tronWeb.defaultAddress.hex, userInfo.wallet))
      return 1;
    else
      return 0;
  }
